...
const resolvers = {
	createGame: {
		success: (req, res, ctx) => {
			return res(ctx.status(200), ctx.delay(mockServiceLatency), ctx.json(createGameSuccess));
		},
	},
	variants: {
		success: (req, res, ctx) => {
			return res(ctx.status(200), ctx.delay(mockServiceLatency), ctx.json(variantsSuccess));
		},
	},
};
